...
# ВИПРАВЛЕНО (зловлено власним "контрольним" тестом при додаванні
# ticket-механізму): попередня версія вимагала ЛИШЕ hex-символи у
# значенні — правильно для 64-hex session token, але
# secrets.token_urlsafe(32) (яким генерується ticket) повертає
# URL-safe base64 (літери поза hex-діапазоном, "-", "_") — реальний
# ticket просто НЕ матчився цим regex і проходив у лог нередагованим.
# Universal-версія: будь-які символи після "=" до розділювача (пробіл,
# "&", лапка) чи кінця рядка — не залежить від конкретного алфавіту
# значення, тож нова зміна формату токена/тикета в майбутньому знову
# не пробʼє захист мовчки.
_TOKEN_QUERY_RE = re.compile(r'([?&](?:token|ticket)=)[^\s&"]+')
