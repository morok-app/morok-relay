...
class MeInfo(BaseModel):
    ...
    tier: str
    created_at: int
    # Чи є релей, що відповідає, ДОМОМ цього акаунта. Дублює логіку
    # auth-відповіді, але тут — для клієнта зі ЗБЕРЕЖЕНОЮ сесією:
    # auth він може не проходити до 30 днів, а /me смикає при кожному
    # відкритті. Банер «ти не вдома» має спиратися саме на це поле —
    # клієнт не мусить сам знати ім'я релея, до якого підключений
    # (через onion це взагалі неочевидно). Адитивно.
    is_home_relay: bool = True
...
class GroupEnvelopeAck(BaseModel):
    envelope_id: str
    queued: bool
    # Кількість одержувачів, яким конверт РЕАЛЬНО поставлено в чергу
    # (локальні: підтверджено синхронно; віддалені: надіслано на
    # federation forward — підтвердження доставки прийде асинхронно,
    # синхронно ми можемо знати лише "спробували", не "доставлено").
    # Аудит зовн. №5, MEDIUM: раніше тут завжди стояло len(group.
    # members)-1 — запитана кількість, що ігнорувала переповнені inbox
    # локальних одержувачів. Сервер не повинен підтверджувати
    # доставку, якої не сталося.
    recipient_count: int
    # True, якщо ХОЧ ОДИН локальний одержувач був пропущений через
    # переповнену чергу (не помилка сервера — просто чужий inbox на
    # межі; клієнт може показати "доставлено не всім" замість тихо
    # довіряти числу).
    some_recipients_skipped: bool = False
    expires_at: int
